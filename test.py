...
while True:
    ...
    corner_points = [polygon[top_left][0], polygon[top_right][0], polygon[bottom_right][0], polygon[bottom_left][0]]

    for point in corner_points:
        frame = cv.circle(frame, tuple(int(x) for x in point),5,(0,255,0),-1)
    
    cv.imshow('frame', frame)

    pts1 = np.float32([polygon[top_left][0],polygon[bottom_left][0], polygon[top_right][0],polygon[bottom_right][0]])
    pts2 = np.float32([[0,0],[522,0],[0,522],[522,522]])

    M = cv.getPerspectiveTransform(pts1, pts2)
    dst = cv.warpPerspective(frame, M, (522,522))

    cv.imshow('dst', dst) 

    preprocess_image = preprocessing_image(dst)
    ...
    display_on_board(main_arr, secondary_arr, preprocess_image)

    cv.imshow('f', preprocess_image)
